...
const Button = ({ title, active, width,onClick,height}: Props) => {
  ...
  return (
    <div >
      <button
        className={`py-1 px-3 border-2 max-md:font-xs font-base rounded-md mr-2  mb-2 flex justify-center items-center ${
          active ? "blue" : "white"
        } ${width && "w-full text-lg font-medium"} ${height && "h-12"} 
           ${isLoading && "cursor-not-allowed bg-gray-300 text-black hover:bg-gray-300"}`}
           onClick={onClick}
        disabled={isLoading}
      >
        {isLoading && (
